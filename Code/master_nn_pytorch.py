...
def master_nn_pytorch(train_set,dev_set,class_name,hidden_activation,output_activation,learning_rate,layer_dims,batch_size,iter_num,optimizer,reg_lambd=0.0,keepnode_prob=1.0,restart='False'):

    filename = output_activation

    if restart=='False':
        torch.save({'train_set':train_set,'dev_set':dev_set}, 'data_split_'+output_activation)
    else:
        data_split = torch.load('data_split_'+output_activation)
        train_set = data_split['train_set']
        dev_set = data_split['dev_set']
    ...
    # I. define model
    model = define_model(output_activation,class_name,layer_dims,keepnode_prob)
    model.train()    

    # II. define optimizer for gradient descent
    ...
    # III. train model    
    for epoch in range(iter_start, iter_num):
        t = 0
        for input, label in train_loader:
            ...
            t = t + 1
            if t%1==0:
                print('cost=',cost.item(),' at batch ', str(t),' epoch ',str(epoch))


    # IV Save model
    if iter_start!=iter_num:
        torch.save({'epoch': epoch+1, 'model_state_dict': model.state_dict(),'optimizer_state_dict': optimizer.state_dict(),'loss': cost}, filename + '_checkpoint.pt')
...
#    in_img_path = '/Users/xl332/Desktop/GamePics/'
